...
class SyncOrdersJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        dump("Job: Начало синхронизации заказов для аккаунта ID {$this->accountId}");

        try {
            $service = new WbReportsService($account);
            $saved = $service->syncOrders();

            dump("Job: Обработано {$saved} записей для аккаунта ID {$this->accountId}");
            Log::info("Синхронизация заказов для аккаунта ID {$this->accountId} завершена. Записей: {$saved}");

        } catch (\Exception $e) {
            Log::error("Ошибка синхронизации заказов для аккаунта ID {$this->accountId}: " . $e->getMessage());
            throw $e;
        }
    }
