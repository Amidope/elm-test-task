...
class SyncIncomesJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        dump("Job: Начало синхронизации поступлений для аккаунта ID {$this->accountId}");

        try {
            $service = new WbReportsService($account);
            $saved = $service->syncIncomes();

            dump("Job: Обработано {$saved} записей для аккаунта ID {$this->accountId}");
            Log::info("Синхронизация поступлений для аккаунта ID {$this->accountId} завершена. Записей: {$saved}");

        } catch (\Exception $e) {
            Log::error("Ошибка синхронизации поступлений для аккаунта ID {$this->accountId}: " . $e->getMessage());
            throw $e;
        }
    }
